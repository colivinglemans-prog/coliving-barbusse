...
const ICONS: React.ReactNode[] = [
  <LinensIcon key="linens" />,
  <CleaningIcon key="cleaning" />,
  <WifiIcon key="wifi" />,
  <TvIcon key="tv" />,
  <DeskIcon key="desk" />,
  <CoffeeIcon key="coffee" />,
  <KitchenIcon key="kitchen" />,
  <WasherIcon key="washer" />,
  <DryerIcon key="dryer" />,
  <GymIcon key="gym" />,
  ...
  <DishwasherIcon key="dishwasher" />,
  <MicrowaveIcon key="microwave" />,
  <OvenIcon key="oven" />,
  <FridgeIcon key="fridge" />,
  <SmokeDetectorIcon key="smoke" />,
  <ExtinguisherIcon key="extinguisher" />,
];

export default function Amenities() {
  const [showAll, setShowAll] = useState(false);
  const { t } = useTranslation();
  const labels = t.amenities.labels;
  const total = labels.length;
  const visible = showAll ? labels : labels.slice(0, 10);

  return (
    <div id="equipements" className="mx-auto max-w-6xl border-b border-border px-6 py-8">
      <h2 className="text-xl font-semibold text-foreground">{t.amenities.title}</h2>
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        {visible.map((label, i) => (
          <div key={label} className="flex items-center gap-4">
            <div className="shrink-0 text-secondary">{ICONS[i]}</div>
      ...
      {total > 10 && (
        ...
        >
          {showAll ? t.amenities.showLess : t.amenities.showAll(total)}
        </button>
      )}
    </div>
  );

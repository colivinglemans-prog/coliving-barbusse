/**
 * Vacances scolaires par zone — **port du module d'Albiez**, volontairement copié plutôt
 * que partagé : les deux sites sont deux dépôts et deux déploiements, sans code commun.
 *
 * Le fichier est tenu identique à `lib/periodes.ts` d'Albiez pour que toute correction se
 * reporte sans réflexion. Les données viennent de `scripts/build-vacances.mjs`, à relancer
 * quand le ministère publie une nouvelle année scolaire.
 *
 * Certaines fonctions (`findPeriodesForStay`, `periodeLabel`) ne sont pas encore utilisées
 * ici : Barbusse étiquette ses séjours par événement du circuit (`lib/events.ts`), pas par
 * période de vacances. Elles restent en place pour ne pas faire diverger les deux copies.
 */

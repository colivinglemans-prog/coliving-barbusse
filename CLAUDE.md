...
app/
  ...
  api/
    ...
    cron/
      ...
      heating-reset/       # Reset modes + températures toutes les 4h
      heating-health/      # Check connectivité + email alertes
components/
  public/             # Composants du site vitrine
  dashboard/          # Composants du dashboard (stats, calendrier, chauffage)
lib/
  ...
  heatzy.ts           # Client API Heatzy/Gizwits (auth, devices, contrôle)
...
- **Rôles auth** : `admin` (accès complet) et `viewer` (calendrier + chauffage)
...
- **Logique horaire** : 7h-20h présence, 20h-7h confort (chambres occupées)
- **Température API** : valeurs en dixièmes (179 = 17.9°C), diviser par 10
- **Alertes** : basées sur température réelle vs consigne + Heating_state
- **Config** : `data/heatzy-zones.json` (device IDs, zones, mapping Beds24)
